...
def sort_input(input_list):
    ...
    for i, elm in enumerate(input_list):
        temp.append(map(int, elm.split('.')))

    sorted_temp = sorted(temp)

    return sorted_temp

def connect_elm(sorted_input):

    versions = []

    for i, elm in enumerate(sorted_input):
        v = '.'.join(map(str, elm))
        versions.append(v)
...
def answer(l):
    ...
    sorted_input = sort_input(input_list)
    result = connect_elm(sorted_input)
